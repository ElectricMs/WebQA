...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 允许的原始域名列表
    allow_credentials=True,  # 允许凭证（cookies、HTTP认证等）
...
async def ask_question(request: Request):
    try:
        ...
        # 使用预加载的RAG模型生成答案
        answer = await loop.run_in_executor(thread_pool, model.generate_answer(question))
                ...
                if "chunk" in chunk["data"]:
                    ...
                    if isinstance(chunk["data"]["chunk"], AIMessageChunk):
                        ...
                        if content:
                            ...
                            elif start_final and ":" in content:
                                start_stream = True
...
@app.get("/directchat")
async def directchat():
    return {"message": "DirectChat still needs further processing"}
